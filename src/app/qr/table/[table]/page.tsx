...
import { qrRuntimeService } from "@/modules/qr-ordering/services/qrRuntime.service";
import {
  AlertTriangle,
  Store,
  Utensils,
...
export default async function QRTablePage({
  ...
  const { table: qrToken } = await params;

  let page;

  try {
    page = await qrRuntimeService.build({
      qrToken,
    });
  } catch {
    return (
